...
def coarseLoss(y_true, y_pred, mask):
    ...
    chan_loss = 0
    for i in range(11):
      chan1 = tf.reduce_mean(tf.abs(y_true[:,:,:,i] - y_pred[:,:,:,i])*(mask[:,:,:])/tf.reduce_mean(tf.reshape(mask[:,:,:], (-1,1,1))))
      chan2 = tf.reduce_mean(tf.abs(y_true[:,:,:,i] - y_pred[:,:,:,i])*(1- mask[:,:,:])/(1-tf.reduce_mean(tf.reshape(mask[:,:,:], (-1,1,1)))))
